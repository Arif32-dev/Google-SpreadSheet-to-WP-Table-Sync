...
class SheetFetching {
    ...
    public static function sheet_html($url, $table_name, $source_type, $table_settings, $table_id) {
        ...
        if (count(fgetcsv($sheet_response)) < 2) {
            self::$output['response_type'] = esc_html__('invalid_request','sheets-to-wp-table-live-sync');
            self::$output['output'] = '<b>' . esc_html__('The spreadsheet is restricted.','sheets-to-wp-table-live-sync') . '<br/>' . esc_html__(' Please make it public by clicking on share button at the top of spreadsheet','sheets-to-wp-table-live-sync') . '</b>';
            return self::$output;
        }
        ...
        self::$output['table_data'] = [
            'table_id' => esc_html__($table_id,'sheets-to-wp-table-live-sync'),
            'source_url' => esc_url($url),
            'table_name' => esc_html__($table_name,'sheets-to-wp-table-live-sync'),
            'source_type' => esc_html__($source_type,'sheets-to-wp-table-live-sync'),
            'table_settings' => json_encode(array_map(function ($setting) {
                return esc_html__($setting,'sheets-to-wp-table-live-sync');
            }, $table_settings))
        ];
